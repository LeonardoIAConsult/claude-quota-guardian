...
| `hooks/enforce-checkpoint.js` | `PreToolUse` | Hard stop: once a checkpoint is pending and unconsumed, blocks every tool call except `Bash`/`Write` (the two the `/continuity-checkpoint` flow needs) until the checkpoint is saved. Scoped to the terminal sessions that are over threshold: the one that tripped it plus any other that crosses it while it is pending (`offenders`); other sessions only get the `PostToolUse` advisory. `/guardian-continue` lifts it for a grace window, and an un-checkpointed pending is released automatically once a real reading of the same signal is back under the threshold (never by elapsed time alone, never on a missing reading). This is a real block — unlike the `PostToolUse`/`Stop` advisories, the tool call never runs. |
...
`getStatus` consults it first: when available, its more-pressing window becomes `planPct`/`planResetAt` directly and the ccusage subprocess is skipped entirely (the statusline `rate_limits` cache below still wins if it reports a *higher* number, e.g. a fresher reading). Refetches are throttled to once every `usageApi.cacheSeconds` (default 60s) **per account**, not per project: the last reading lives in one shared file, `~/.claude/session-continuity/api-usage.json`. After a failed request (e.g. HTTP 429) every session backs off together, exponentially (2 min, 4 min, 8 min, capped at 15 min), serving the last good reading meanwhile. Readings from disk are sanitized (a future `fetchedAt`, a pct outside 0-100 or malformed windows are discarded). Readings are force-refreshed when a window's `resets_at` passes (rollover), and a stale reading is reused when a due refresh fails — unless its window already rolled over.
