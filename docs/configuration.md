...
| `thresholds.desktopWarn` | `0.99` | Notify-only warning threshold for the Claude Code Desktop surface (context %). Desktop also gets an account-quota warn at `thresholds.plan`. Desktop is never hard-blocked. Set to `null` to silence Desktop entirely. |
