...
function readSessionUsage(file, mtimeMs) {
  ...
  return {
    ...
    projectPath,
    // Split on both separators: path.basename is platform-bound, and a rollout
    // recorded on Windows (C:\proj\demo) can be read on POSIX and vice versa.
    projectName: projectPath ? projectPath.split(/[\\/]/).filter(Boolean).pop() || null : null,
