...
// Absolute path, never the bare name: Windows looks for an executable in the
// current directory before PATH, so a bare `schtasks` could run a planted
// schtasks.exe. path.win32 keeps the result identical when tests run on POSIX.
function schtasksExe() {
  // A relative SystemRoot would bring the current-directory lookup back.
  const root = process.env.SystemRoot;
  const base = root && path.win32.isAbsolute(root) ? root : 'C:\\Windows';
  return path.win32.join(base, 'System32', 'schtasks.exe');
}
...
function describeInstall(platform, { nodePath, watcherPath, intervalMinutes, logPath }) {
  if (platform === 'win32') {
    return {
      ...
      files: [],
      commands: [
        [schtasksExe(), '/create', '/tn', TASK_NAME, '/tr', `"${nodePath}" "${watcherPath}"`, '/sc', 'minute', '/mo', String(intervalMinutes), '/f'],
      ],
    };
  }
...
function describeReschedule(platform, opts) {
  if (platform === 'win32') {
    return {
      platform: 'win32',
      commands: [
        [schtasksExe(), '/create', '/tn', TASK_NAME, '/tr', `"${opts.nodePath}" "${opts.watcherPath}"`, '/sc', 'minute', '/mo', String(opts.intervalMinutes), '/f'],
      ],
    };
  }
...
function describeUninstall(platform) {
  if (platform === 'win32') {
    return {
      platform: 'win32',
      commands: [[schtasksExe(), '/delete', '/tn', TASK_NAME, '/f']],
      filesToRemove: [],
    };
  }
...
module.exports = { describeInstall, describeReschedule, describeUninstall, pickIntervalMinutes, schtasksExe, TASK_NAME, LAUNCHD_LABEL };
