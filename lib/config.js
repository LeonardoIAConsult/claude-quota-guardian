...
const DEFAULTS = {
  ...
  // Default false: blocking is driven 100% by the real account plan quota
  // (usageApi / rate_limits), never by the local transcript context estimate --
  // which overshoots on large/1M-token windows and is redundant now that Claude
  // Code auto-compacts context. The context% is still measured and reported
  // (statusline/heartbeat), just never a blocking signal. Set to true to opt back
  // in: then the context estimate acts as a FALLBACK block, but only when no real
  // quota signal is available (see lib/usage-monitor.js).
  blockOnContext: false,
