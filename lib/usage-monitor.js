...
// Guardian's own install dir (see runCcusage).
const TRUSTED_CWD = path.join(__dirname, '..');
  ...
  for (let i = lines.length - 1; i >= 0; i--) {
    ...
    if (contextResult === null) {
      ...
      if (usage) {
        ...
        // A context estimate is a fraction of the window, so >100% is impossible:
        // it means the real window is larger than our assumed `limit` (e.g. the
        // 1M-token beta context) and the denominator is wrong. Never drive a hard
        // block off an impossible reading -- treat it as unknown so the plan-quota
        // guard owns the decision instead of the estimate false-firing every turn.
        contextResult = rawPct > 100
          ? { pct: null, used, limit, model, error: 'estimate-exceeds-window' }
          : { pct: rawPct, used, limit, model };
      }
    }
...
function runCcusage(args) {
  try {
    ...
    // cwd is pinned to Guardian's own install dir: with shell:true, cmd.exe
    // resolves `npx` from the CURRENT directory before PATH, and the hook's
    // inherited cwd is the (untrusted) project -- a repo shipping npx.cmd in
    // its root would otherwise run as soon as a hook fired.
...
function getStatus({ transcriptPath, config, cachedRateLimit, cachedPlan, planCheckCounter, cachedApiUsage, samples = [], now = Date.now() }) {
  ...
  // A cached value whose window already reset is not a reading of anything:
  // counting it could keep a block up forever (stale high) or release one
  // wrongly (stale low). Same rule as the usage-API cache's rollover check.
  const rateLimitLive = cachedRateLimit && typeof cachedRateLimit.pct === 'number'
    && !(Date.parse(cachedRateLimit.resetAt) <= now);
  if (rateLimitLive && (planPct === null || cachedRateLimit.pct > planPct)) {
  ...
  // The transcript context estimate is a FALLBACK signal. It only hard-blocks
  // when there is no real account-quota reading (usageApi / rate_limits /
  // ccusage): with a real quota signal, the quota owns the block and the flaky
  // estimate is demoted, because used/limit overshoots on large/1M-token windows
  // and would otherwise block prematurely while plenty of quota remains. Setting
  // `blockOnContext: false` disables the fallback entirely. The context% is still
  // measured and reported (statusline/heartbeat) in every case.
  const planSignalAvailable = planPct !== null;
  const contextBlocks = config.blockOnContext !== false && !planSignalAvailable;
  const contextHit = contextBlocks && contextPct !== null && (contextPct / 100) >= config.thresholds.context;
  ...
  const predictedHit = contextBlocks && prediction.hit === true;
