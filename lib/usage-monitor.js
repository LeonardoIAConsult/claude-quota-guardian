...
function getStatus({ transcriptPath, config, cachedRateLimit, cachedPlan, planCheckCounter, cachedApiUsage, samples = [], now = Date.now() }) {
  ...
  // `blockOnContext: false` demotes the local context estimate from a blocking
  // signal: it is still measured/reported below, but never contributes to a
  // block or predictive block, so only the real plan quota can halt the turn.
  const contextBlocks = config.blockOnContext !== false;
