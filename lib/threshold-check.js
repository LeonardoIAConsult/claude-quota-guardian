...
function computeDowngradeWarn({ config, status, prevState, now }) {
  ...
  if (!dg || dg.enabled !== true || status.planPct === null) return { lastDowngradeWarnAt };
  ...
  const inBand = status.planPct >= dg.warnPct && status.planPct < hardPlanPct;
  ...
  if (now - last >= RENOTIFY_INTERVAL_MS) {
    ...
    const pctShown = Math.round(status.planPct * 10) / 10;
    notify.send('Claude Quota Guardian', `Cuota al ${pctShown}% y estás en un modelo premium (${status.model}). Bajá a Sonnet con /model para estirar la cuota — Guardian no lo cambia solo.`, config);
  }
...
function performCheck(input, { config }) {
  ...
  if (!alreadyPending) {
    ...
    notify.send('Claude Quota Guardian', `Uso al ${pctShown}% — guardando progreso...`, config);
  } else {
    ...
    if (!pending.checkpointFile && Date.now() - last >= RENOTIFY_INTERVAL_MS) {
      ...
      notify.send('Claude Quota Guardian', `Seguís sobre el umbral (${pctShown}%) — falta /continuity-checkpoint.`, config);
    }
  }
