...
function releaseRecoveredPending({ pending, pendingFile, status, config, sessionId }) {
  if (!pending || pending.consumed !== false || pending.checkpointFile) return;
  ...
  try {
    atomicWriteFileSync(pendingFile, JSON.stringify({
      ...
      consumedReason: 'below-threshold',
    }, null, 2));
  } catch {
...
function performCheck(input, { config }) {
  ...
  if (!status.anyAtThreshold) {
    releaseRecoveredPending({ pending, pendingFile, status, config, sessionId: input.session_id || null });
    return null;
  }
  ...
  if (!alreadyPending) {
    ...
    if (sid && pending.sessionId && sid !== pending.sessionId && !offenders.includes(sid)) {
      next = { ...next, offenders: [...offenders, sid].slice(-MAX_OFFENDERS) };
    }
    ...
    if (next !== pending) atomicWriteFileSync(pendingFile, JSON.stringify(next, null, 2));
