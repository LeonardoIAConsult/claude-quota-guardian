...
function releaseRecoveredPending({ pending, pendingFile, status, config, sessionId }) {
  ...
  const byContext = /^context/.test(pending.triggeredBy || '');
  const measured = byContext
...
// An offender is one only while it is itself over threshold. Needs a real
// reading, same as a release: silence is not evidence it recovered.
function dropRecoveredOffender({ pendingFile, status, config, sessionId }) {
  if (!sessionId) return;
  const measured = status.planPct !== null
    || (config.blockOnContext !== false && status.contextPct !== null);
  if (!measured) return;
...
// /continuity-checkpoint may have stored checkpointFile. Re-reading here keeps
// that write from being clobbered. `change` returns the new object or null.
...
function performCheck(input, { config }) {
  ...
  if (!status.anyAtThreshold) {
    ...
    if (!releaseRecoveredPending({ pending, pendingFile, status, config, sessionId })) {
      dropRecoveredOffender({ pendingFile, status, config, sessionId });
    }
    return null;
  }
  ...
  if (!alreadyPending) {
    ...
    const renotify = !pending.checkpointFile && Date.now() - last >= RENOTIFY_INTERVAL_MS;

    if (addOffender || renotify) {
      updatePending(pendingFile, (fresh) => {
