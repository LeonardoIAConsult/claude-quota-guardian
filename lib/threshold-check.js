...
// affordance the way a terminal does. Two independent warnings, each with its
// own re-notify timestamp, since the fix differs by signal:
// - local context (config.thresholds.desktopWarn): the fix is opening a fresh
//   conversation, since that resets context to zero.
// - account-wide plan/rate-limit (config.thresholds.plan): opening a new
//   conversation does NOT help -- the quota is account-wide, so the only fix
//   is waiting for the reset.
// Neither path ever creates a pending checkpoint, even past the CLI's hard
// threshold -- Desktop is notify-only by design.
function performDesktopWarn({ config, status, prevState }) {
  ...
  const contextWarnHit = status.contextPct !== null && (status.contextPct / 100) >= config.thresholds.desktopWarn;
  if (contextWarnHit) {
  ...
  if (planWarnHit) {
    ...
    if (Date.now() - last >= RENOTIFY_INTERVAL_MS) {
      ...
      notify.send('Claude Quota Guardian', `Cuota de tu cuenta Pro al ${pctShown}%${resetTxt} — pausá hasta el reset, abrir otra conversación no libera esta cuota.`);
    }
  }
