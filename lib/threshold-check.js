...
function computeScopedWarn({ config, status, prevState, now }) {
  let lastScopedWarnAt = (prevState && prevState.lastScopedWarnAt) || null;
  const sc = config.scoped;
  if (!sc || sc.enabled === false) return { lastScopedWarnAt };
  ...
  if (!hot.length) return { lastScopedWarnAt };

  const last = lastScopedWarnAt ? new Date(lastScopedWarnAt).getTime() : 0;
  if (now - last >= RENOTIFY_INTERVAL_MS) {
    lastScopedWarnAt = new Date(now).toISOString();
...
function performCheck(input, { config }) {
  ...
  const { lastScopedWarnAt } = isCli
    ? computeScopedWarn({ config, status, prevState, now })
    : { lastScopedWarnAt: (prevState && prevState.lastScopedWarnAt) || null };
