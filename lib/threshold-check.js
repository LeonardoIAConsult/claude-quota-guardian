...
// A predictive context pending is created while context is still UNDER the
// threshold, so "under threshold" alone would release it the moment the slope
// flattens. Require context to have actually come down since the trigger
// (e.g. after a compaction).
function contextDroppedSinceTrigger(pending, contextPct) {
  const atTrigger = pending.pctAtTrigger && pending.pctAtTrigger.context;
  return typeof atTrigger !== 'number' || contextPct < atTrigger;
}

function trippedByContext(pending) {
  return /^context/.test(pending.triggeredBy || '');
}

// An un-checkpointed pending used to outlive the quota pressure that created
// it: nothing cleared it, so its session stayed hard-blocked even after the
// quota reset. Release it once a REAL reading shows usage back under the
// threshold. No reading (API down, no creds) never releases -- silence is not
// evidence the quota recovered -- and the reading must be of the SAME signal
// that tripped it (a low context% says nothing about a quota-triggered block).
// A checkpointed pending is left alone: it waits for SessionStart to resume it.
// Quota is account-wide, so any session's reading counts; context% belongs to
// one session, so only the session that tripped a context pending can release it.
function releaseRecoveredPending({ pending, pendingFile, status, config, sessionId }) {
  if (!pending || pending.consumed !== false || pending.checkpointFile) return false;
  const measured = trippedByContext(pending)
    ? config.blockOnContext !== false && status.contextPct !== null
      && (status.contextPct / 100) < config.thresholds.context
      && contextDroppedSinceTrigger(pending, status.contextPct)
      && Boolean(pending.sessionId) && pending.sessionId === sessionId
    : status.planPct !== null;
  if (!measured) return false;
  // Only release the pending this decision was made about: if it was consumed
  // and replaced by a new one while usage was being fetched, leave that alone.
  return updatePending(pendingFile, (fresh) => (fresh.consumed === false && !fresh.checkpointFile
    && fresh.triggeredAt === pending.triggeredAt && fresh.sessionId === pending.sessionId
    ? { ...fresh, consumed: true, consumedAt: new Date().toISOString(), consumedReason: 'below-threshold' }
    : null));
}

// An offender is one only while it is itself over threshold, and it may have
// been recorded for either signal. Same evidence rule as a release: whenever a
// quota signal is configured, a real quota reading is required (a low context%
// says nothing about the quota); context alone only counts when it is the sole
// blocking signal.
function dropRecoveredOffender({ pending, pendingFile, status, config, sessionId }) {
  if (!sessionId || !pending) return;
  const quotaConfigured = (config.usageApi && config.usageApi.enabled === true)
    || (config.plan && config.plan !== 'none');
  const measured = quotaConfigured
    ? status.planPct !== null
    : config.blockOnContext !== false && status.contextPct !== null;
  if (!measured) return;
  updatePending(pendingFile, (fresh) => (fresh.consumed === false
    && Array.isArray(fresh.offenders) && fresh.offenders.includes(sessionId)
    ? { ...fresh, offenders: fresh.offenders.filter((s) => s !== sessionId) }
    : null));
}

// Read-modify-write against the CURRENT pending.json. Callers decide from a
// copy read before getStatus, which can take seconds (usage fetch); meanwhile
// /continuity-checkpoint may have stored checkpointFile. Re-reading here
// narrows that window from seconds to milliseconds (there is no lock).
// `change` returns the new object or null. Returns whether it wrote.
function updatePending(pendingFile, change) {
  const fresh = readJsonSafe(pendingFile);
  if (!fresh) return false;
  const next = change(fresh);
  if (!next) return false;
  try {
    atomicWriteFileSync(pendingFile, JSON.stringify(next, null, 2));
    return true;
  } catch {
    return false; // best-effort: the next check retries
  }
}
...
function performCheck(input, { config }) {
  ...
  if (!status.anyAtThreshold) {
    ...
    if (!releaseRecoveredPending({ pending, pendingFile, status, config, sessionId })) {
      dropRecoveredOffender({ pending, pendingFile, status, config, sessionId });
    }
    return null;
  }
  ...
  if (!alreadyPending) {
    ...
    // The pending holds one slot per project, owned by the session that
    // tripped it. Another terminal session that is ITSELF over threshold
    // (e.g. a new session opened after the owner closed without a checkpoint,
    // or after /clear) is an offender too: record it so enforce blocks it as
    // well. Only this CLI path gets here, so desktop/SDK routines never do.
    const sid = input.session_id || null;
    const offenders = Array.isArray(pending.offenders) ? pending.offenders : [];
    const addOffender = Boolean(sid && pending.sessionId && sid !== pending.sessionId && !offenders.includes(sid));
    ...
    // Decided on the FRESH pending (a checkpoint may have landed during the
    // usage fetch), and only announced once the new lastNotifiedAt is stored --
    // a failing write must not turn into a toast on every tool call.
    const renotifyDue = (p) => {
      const last = p.lastNotifiedAt ? new Date(p.lastNotifiedAt).getTime() : 0;
      return !p.checkpointFile && Date.now() - last >= RENOTIFY_INTERVAL_MS;
    };
    let renotify = false;

    if (addOffender || renotifyDue(pending)) {
      const wrote = updatePending(pendingFile, (fresh) => {
        if (fresh.consumed !== false) return null;
        let next = fresh;
        const current = Array.isArray(fresh.offenders) ? fresh.offenders : [];
        if (addOffender && !current.includes(sid)) {
          next = { ...next, offenders: [...current, sid].slice(-MAX_OFFENDERS) };
        }
        renotify = renotifyDue(fresh);
        if (renotify) next = { ...next, lastNotifiedAt: new Date().toISOString() };
        return next === fresh ? null : next;
      });
      renotify = renotify && wrote;
    }
