...
Write to the `checkpoint` path from Step 1 — **only that exact path**: a checkpoint saved anywhere else (e.g. a CHECKPOINT.md in the repo) is NOT auto-loaded when the next session starts — with this exact structure (fill in real content from this session — no placeholders).
